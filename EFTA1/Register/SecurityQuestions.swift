...
struct SecurityQuestions: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            VStack {
                ScrollView {
                    LogoAndTitleView(geometry: geometry, title: "Set your security questions", subTitle: "Kindly set security questions to be able to reset your PIN in case you forget", presentationMode: presentationMode, goBack: goback)
    ...
    private func submitPinData() {
        ...
        NetworkManager.shared.submitSecurityQuestions(answers: answers, phoneNumber: pinHandler.phoneNumber) { success, error in
            if success {
                        ...
                        showingAlert = true
                        alertMessage = "Failed to submit security questions. Error: \(errorMessage)"
                    }
        }
    }
}
